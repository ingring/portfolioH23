<template>
    <div class="flex justify-center h-screen items-center bg-yellow md:bg-gradient-to-r md:from-yellow md:to-red text-black">
        <div class="flex flex-col m-5 md:m-6">
            <p class="text-3xl md:text-5xl lg:text-6xl">Hi, I'm Ingrid</p>
            <h1 class="text-7xl md:text-[12rem] lg:text-[16rem] flex">Gladhe        
                <div class="relative">
                    <span class="text-7xl md:text-[12rem] lg:text-[16rem]">i</span>
                    <div class="absolute top-0 left-1/5 w-5 h-5 md:top-4 md:left-1/4 md:w-8 md:h-8 lg:w-10 lg:h-10 lg:top-7 md:bg-gradient-to-r md:from-[#f2ab4b] md:to-[#f1a94a] md:rounded-full bg-yellow"></div>
                    <img src="/public/smileyBlackBg.png" class="absolute top-0 left-1/5 w-5 h-5 md:top-4 md:left-1/4 md:w-8 md:h-8 lg:w-10 lg:h-10 lg:top-7">
                </div>
            m
            </h1>
            <p class=" py-5 text-2xl md:text-4xl max-w-prose">I'm a <span class="font-bold ">dedicated</span> norwegian NTNU student in <span class="font-bold ">web development</span> who finds <span class="font-bold">joy</span> and a sense of <span class="font-bold">accomplishment</span> in every coding adventures.</p>
        </div>
    </div>
</template>
